...
class MainViewModel(private val sharedPreferences:
    ...
    //lambda to inform interested classes when a list is added to app
    lateinit var onListAdded: (() -> Unit)

    val lists: MutableList<TaskList> by lazy {
        retrieveLists()
    }
    //getting all saved TaskLists from SharedPreferences
    private fun retrieveLists(): MutableList<TaskList> {
        val sharedPreferencesContents = sharedPreferences.all
        val taskLists = ArrayList<TaskList>()
        //looping through & recreating objects from HashSets
